...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ========== MANEJO DE CLICKS DELEGADOS (IMÁGENES, EDICIÓN, ELIMINACIÓN) ==========
    if (commentsContainer) {
        commentsContainer.onclick = function (e) {
            // 1. Ver imagen comentario
            ...
            // 2. Editar comentario
            const btnEdit = e.target.closest('.btn-edit-comment-client');
            if (btnEdit) {
                const comentarioId = btnEdit.dataset.id;
                const comentarioItem = btnEdit.closest('.comment-item-wrapper');
            ...
            const btnDelete = e.target.closest('.btn-delete-comment-client');
            if (btnDelete) {
                const comentarioId = btnDelete.dataset.id;
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        ...
                                        const comentarioItem = btnDelete.closest('.comment-item-wrapper');
                                        if (!comentarioItem) {
                                        ...
                                        setTimeout(() => {
                                            ...
                                            if (commentsContainer.querySelectorAll('.comment-item-wrapper').length === 0) {
                                                commentsContainer.innerHTML = '<div id="no-comments-msg" class="fade-in-up" style="margin-top: 0.5rem; color: var(--text-secondary);"><i class="fas fa-comment-dots"></i> Sin comentarios todavía</div>';
                                            }
                                        }, 400);
                                    });
                                } else {
